...
public class BaseRapidStyleMigrationAllImageFetch implements IAllImageFetch {
    ...
    private void init() {
        ...
        inferenceInterface = new TensorFlowInferenceInterface(
                SmartGalleryApplication.getAppContext().getAssets(),
                mModelFile);
        MyLog.d(TAG, "BaseRapidStyleMigrationAllImageFetch", "状态:初始化时间",
                "快速风格迁移初始化时间", System.currentTimeMillis() - initTime);
    ...
    public Mat run (Mat oldMat) {
        ...
        middleMat.release();
        MyLog.d(TAG, "BaseRapidStyleMigrationAllImageFetch", "状态:图片时间",
                "快速风格迁移读取图片数据时间", System.currentTimeMillis() - initTime);
        ...
        MyLog.d(TAG, "BaseRapidStyleMigrationAllImageFetch", "状态:运行时间",
                "快速风格迁移运行时间", System.currentTimeMillis() - initTime);

        initTime = System.currentTimeMillis();
        for (int i = 0; i < mOutFloatValues.length; i++) {
            mOutIntValues[i] =
                    0xFF000000
        ...
        result.setPixels(mOutIntValues, 0, result.getWidth(), 0, 0,
        ...
        middleMat2.release();
        result.recycle();
        MyLog.d(TAG, "BaseRapidStyleMigrationAllImageFetch", "状态:传出数据时间",
                "快速风格迁移传出数据时间", System.currentTimeMillis() - initTime);
        return outMat;
    }
